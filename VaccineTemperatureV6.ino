...
// [NEW] เพิ่มไลบรารีสำหรับจัดการพลังงาน WiFi
#include <esp_wifi.h> 
...
// ================= Setup & Loop =================
void setup() {
  // [NEW] ลดความเร็ว CPU จาก 240MHz เป็น 80MHz เพื่อประหยัดพลังงาน (WiFi ยังทำงานได้ปกติ)
  ...
  display.dim(true); // [NEW] ลดความสว่างจอ OLED ลงเพื่อประหยัดแบต/ไฟ
  ...
  // [NEW] เปิดใช้งาน WiFi Modem Sleep (ประหยัดพลังงานเมื่อไม่มีการส่งข้อมูล)
...
void loop() {
  ...
  static float lastDisplayedTemp = -999.0; // [NEW] ตัวแปรจำค่าจอที่แสดงล่าสุด
  ...
  if (isRequestingTemp && (currentMillis - prevTempM >= 750)) {
    ...
    // [NEW] อัปเดตหน้าจอเฉพาะเมื่ออุณหภูมิเปลี่ยนไปจากเดิมเท่านั้น (ลดภาระ CPU และ I2C)
  ...
  if (currentTemp != 0.0 && currentTemp != -127.00 && currentTemp != 85.00) {
      ...
      if (scheduleTimes.indexOf(curTime) >= 0 && lastSentMinute != curTime) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
          if(sendToGoogleSheets(String(fullTime), currentTemp) && !offlineBuffer.empty()) {
            for(auto& log : offlineBuffer) { 
              sendToGoogleSheets(log.timestamp, log.temperature);
              delay(500); 
            }
            offlineBuffer.clear();
          }
  ...
  // [NEW] คืนเวลาให้ FreeRTOS (OS พื้นฐานของ ESP32) เพื่อให้ระบบเข้าสู่ Automatic Light Sleep
  delay(10); 
}