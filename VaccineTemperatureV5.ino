...
// อย่าลืมแก้ไขข้อมูลตรงส่วนนี้ให้เป็นของคุณก่อนอัปโหลด
const char* ssid = "*****";
const char* password = "*****";
...
void handleSettings() {
  ...
  // รับค่าการเปิด-ปิดแจ้งเตือน LINE
...
void loop() {
  ...
  // 2. แจ้งเตือน LINE Bot (มีเงื่อนไขเปิด-ปิดเพิ่มเข้ามา)
  if (currentTemp != 0.0 && currentTemp != -127.00 && currentTemp != 85.00) {
    if (currentMillis - lastAlertCheckMillis >= 2 * 60 * 1000) {
      ...
      if (currentTemp < tempMin || currentTemp > tempMax) {
        // เช็คว่าผู้ใช้เปิดแจ้งเตือน LINE ไว้หรือไม่
        if (enableLineNotify) {
